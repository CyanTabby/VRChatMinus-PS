...
If (-Not ($LibraryFolders -Match "(?s)path`"\s*`"(?:(?!path).)*path`"\s*`"(?<librarypath>.*?)`".*$AppId")) {
...
#                                                           |           |           |
#                                                           V           V           V
$Shortcut.Arguments = " -Command `"& '.\VRChatMinus-PS.ps1' --profile=0 --fps=144 --enable-debug-gui --enable-sdk-log-levels --log-debug-levels=API --osc=9000:127.0.0.1:9001`""
